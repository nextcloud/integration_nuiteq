OC.L10N.register(
    "integration_nuiteq",
    {
    "Bad HTTP method" : "Blogas HTTP metodas",
    "Bad credentials" : "Blogi prisijungimo duomenys",
    "Invalid response" : "Neteisingas atsakymas",
    "Invalid credentials" : "Neteisingi prisijungimo duomenys",
    "Connected accounts" : "Prijungtos paskyros",
    "NUITEQ Stage whiteboard integration" : "„NUITEQ Stage“ integracija su skaitmenine lenta",
    "Nuiteq" : "„Nuiteq“",
    "Quickly create and share NUITEQ Stage boards in Nextcloud" : "Greitai kurkite ir bendrinkite „NUITEQ Stage“ lentas „Nextcloud“ sistemoje",
    "NUITEQ Stage creates a natural interaction between participants in a Modern Meeting by merging the experience of creating together on a whiteboard and discussing in a video call and chat. Prepare and upload files and presentations, share them with others in the meeting. Designed for collaboration first to engage team members - NUITEQ Stage is safe, easy and efficient to use.\n\nThis app provides a simple interface to list, create and share NUITEQ Stage whiteboards." : "„NUITEQ Stage“ užtikrina natūralią dalyvių sąveiką šiuolaikiniame susitikime, sujungdama bendro kūrybinio darbo ant skaitmeninės lentos patirtį su diskusijomis vaizdo skambučio ir pokalbių metu. Paruoškite ir įkelkite failus bei pristatymus, dalinkitės jais su kitais susitikimo dalyviais. Sukurta pirmiausia bendradarbiavimui, siekiant įtraukti komandos narius – „NUITEQ Stage“ yra saugi, paprasta ir efektyvi naudoti.\n\nŠi programa siūlo paprastą sąsają, skirtą „NUITEQ Stage“ skaitmeninių lentų peržiūrai, kūrimui ir dalijimuisi.",
    "Failed to get boards" : "Nepavyko gauti lentų",
    "New board was created in NUITEQ stage" : "„NUITEQ Stage“ platformoje sukurta nauja lenta",
    "Failed to create new board" : "Nepavyko sukurti naujos lentos",
    "Failed to delete the board" : "Nepavyko ištrinti lentos",
    "{name} deleted" : "{name} ištrintas",
    "You are not connected to NUITEQ Stage" : "Nesate prisijungę prie „NUITEQ Stage“",
    "You haven't created any boards yet" : "Dar nesukūrėte jokių lentų",
    "Create a board" : "Sukurti lentą",
    "No selected board" : "Nėra pasirinktos lentos",
    "Nuiteq admin options saved" : "„Nuiteq“ administratoriaus parinktys išsaugotos",
    "Failed to save Nuiteq admin options" : "Nepavyko išsaugoti „Nuiteq“ administratoriaus parinkčių",
    "Nuiteq integration" : "„Nuiteq“ integracija",
    "Default NUITEQ Stage URL for users" : "Numatytasis „NUITEQ Stage“ URL vartotojams",
    "Default NUITEQ Stage client key for users" : "Numatytasis „NUITEQ Stage“ kliento raktas vartotojams",
    "Client key" : "Kliento raktas",
    "Leave this empty to use the default client key." : "Palikite šį lauką tuščią, jei norite naudoti numatytąjį kliento raktą.",
    "Public link copied!" : "Viešoji nuoroda nukopijuota!",
    "Link could not be copied to clipboard" : "Nepavyko nukopijuoti nuorodos į iškarpinę",
    "Open here" : "Atverti čia",
    "Open in a new tab" : "Atverti naujoje kortelėje",
    "Share link to a Talk conversation" : "Pasidalinti nuoroda į „Pokalbiai“ pasikalbėjimą",
    "Public board link" : "Viešosios lentos nuoroda",
    "Copy to clipboard" : "Kopijuoti į iškarpinę",
    "Enabled" : "Įjungta",
    "Disabled" : "Išjungta",
    "Delete" : "Ištrinti",
    "Field \"{name}\" is missing" : "Trūksta lauko „{name}“",
    "Choose color" : "Pasirinkite spalvą",
    "Cancel" : "Atsisakyti",
    "Create" : "Sukurti",
    "Successfully connected to NUITEQ!" : "Sėkmingai prisijungta prie NUITEQ!",
    "Nuiteq options saved" : "„Nuiteq“ parinktys išsaugotos",
    "Failed to save NUITEQ options" : "Nepavyko išsaugoti „NUITEQ“ parinkčių",
    "NUITEQ Stage URL" : "„NUITEQ Stage“ URL",
    "Leave the client key empty to use the default one." : "Palikite kliento raktą tuščią, kad būtų naudojamas numatytasis.",
    "Login" : "Prisijungti",
    "NUITEQ login" : "„NUITEQ“ prisijungimas",
    "Password" : "Slaptažodis",
    "NUITEQ password" : "„NUITEQ“ slaptažodis",
    "Connect to NUITEQ Stage" : "Prijunkite prie „NUITEQ Stage“",
    "Connected as {user}" : "Prisijungta kaip {user}",
    "Disconnect from NUITEQ" : "Atsijungti nuo „NUITEQ“",
    "You created a conversation with {name}" : "Jūs pradėjote pokalbį su {name}",
    "Failed to join" : "Nepavyko prisijungti",
    "You joined {name}" : "Jūs prisijungėte prie {name}",
    "NUITEQ board \"{name}\": {link}" : "NUITEQ lenta „{name}“: {link}",
    "Link sent to {name}" : "Nuoroda išsiųsta - {name}",
    "Failed to send link" : "Nepavyko išsiųsti nuorodos",
    "Send link to a Talk room" : "Siųsti nuorodą į „Pokalbiai“ kambarį",
    "Search for users, groups or conversations" : "Ieškoti vartotojų, grupių ar pokalbių",
    "Conversations" : "Pokalbiai",
    "Users" : "Vartotojai",
    "Groups" : "Grupės",
    "Send" : "Siųsti",
    "Board name" : "Lentos pavadinimas",
    "board name" : "lentos pavadinimas",
    "New board" : "Nauja lenta"
},
"nplurals=4; plural=(n % 10 == 1 && (n % 100 > 19 || n % 100 < 11) ? 0 : (n % 10 >= 2 && n % 10 <=9) && (n % 100 > 19 || n % 100 < 11) ? 1 : n % 1 != 0 ? 2: 3);");
