...
namespace OCA\Nuiteq\Migration;
...
class Version1200Date202410151354 extends SimpleMigrationStep {
	...
	public function postSchemaChange(IOutput $output, Closure $schemaClosure, array $options) {
		// encrypt "client_key" in the appconfig
		$clientSecret = $this->config->getAppValue(Application::APP_ID, 'client_key');
		if ($clientSecret !== '') {
			$clientSecret = $this->crypto->encrypt($clientSecret);
			$this->config->setAppValue(Application::APP_ID, 'client_key', $clientSecret);
		}

		// encrypt user's "client_key"
		...
		$qbSelect->andWhere(
			$qbSelect->expr()->eq('configkey', $qbSelect->createNamedParameter('client_key', IQueryBuilder::PARAM_STR))
		);
